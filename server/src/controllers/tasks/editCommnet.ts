...
import { CommentsRepository } from "../../repositories/CommnetRepo";

export const editCommnet = async (req:Request, res:Response) => {
	try {
		...
		const commentRepo = getCustomRepository(CommentsRepository);
		await commentRepo.editCommnet(uuid, commentId, content);
