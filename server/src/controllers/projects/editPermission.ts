...
export const editPermisison = async(req: Request, res: Response) => {
	try {
		const { projectId, email, isAdmin } = req.body;
		...
		projPermissionRepo.editPermission(uuid, projectId, email, isAdmin)
			.then(data => {
				console.log(data);
