...
export const editSec = async (req:Request, res:Response) => {
	try {
		if(!req.cookies) throw errorTypes.LOGIN_IS_REQUIRED;
		const { projectId, sectionId, title } = req.body;
		...
		const data = await sectionRepo.editSect(uuid, projectId, sectionId, title);
